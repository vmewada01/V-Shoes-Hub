...
import { Box, Button, Flex, Heading, Image, Spinner, Table, TableCaption, TableContainer, Tbody, Td, Text, Th, Thead, Tr } from '@chakra-ui/react'
...
// const [currentImg, setCurrentImg] = useState(picture[0])
  ...
  return (
    ...
    <Box w={{lg:"80%"}} m="auto" pt={"20px"} pb="20px" >
          {data.isloading && <SpinnerIcon />}
          {data.isError &&   <Text>something went wrong</Text>  }
      {item && 
...

    {
       item?.images?.map((ele)=> {
           ...
           <Box key={ele.id} border="1px solid #e1e1e1" w={"20%"} p="8px" m={"auto"}  >
